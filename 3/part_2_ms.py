...
def main():
        ...
        for index in range(len(line_list[line_number])):
            ...
            # If it is not a digit and there is a current number with a part associated add it to the list of parts. Then reset current number
                ...
                if current_number and is_part:
                    ...
                    parts.append([current_number,char_coord])
                current_number = ""
                is_part = False
                continue
            ...
            for line_iterator in range(-1,2):
                ...
                for adjacent_iterator in range(index-1,index+2,1):
                    ...
                    if line_list[lineindex][adjacent_iterator] in special_characters:
                        char_coord = [lineindex,adjacent_iterator]
    ...
    [print(f'{i[0]} and {j[0]} have the same gear') for i in parts for j in parts if i[1] == j[1] and i[0] != j[0]]
    ratios = [ [int(i[0])*int(j[0])] for i in parts for j in parts if i[1] == j[1] and i[0] != j[0]]
    total = 0
    for ratio in ratios:
        result = reduce(operator.mul, ratio, 1)
        total = total + result 
    print(total/2)
